...
<?php
	if(isset($_SESSION['cart_array']))
	{
		...
		foreach ($_SESSION['cart_array'] as $cart_itm)
		{
			...
			$query = mysql_query("SELECT * FROM Product_Table WHERE ProductSKU = '$productSKU'", $connection);
			$results = mysql_fetch_assoc($query);
			$subtotal = ($results['ProductPrice'] * $quantity);
			$bg_color = ($b++%2==1) ? 'odd' : 'even'; //zebra stripe
			echo '<tr class="'.$bg_color.'">';
			echo '<td>'.$results['ProductName'].'</td>';
			echo '<td>'.$results['Size'].'</td>';
			echo '<td>'.$results['Color'].'</td>';
			echo '<td>$'.$results['ProductPrice'].'</td>';
			echo '<td>'.$quantity.'</td>';
			echo '<td>$'.$subtotal.'</td>';
			echo '</tr>';
...
?>
<p>Order Total: <?php echo $total_message; ?></p> 
<form action="" method="post">
	<input type="submit" name = "submit" value="Continue to Checkout" />
</form>
<p><?php echo $message; ?></p>
<form action="Xclearcart.php">
