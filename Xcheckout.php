...
<?php
	...
	if (isset($_POST['submit'])) {
		...
		$experation=$_POST['month'].$_POST['year'];
		...
		// create get customer info
		$query1 = mysql_query("SELECT * FROM Customer_Table WHERE CustomerID = '$customerID'", $connection);
		$results1 = mysql_fetch_assoc($query1);
		$address = $results1['StreetAddress'];
		$city = $results1['City'];
		$state = $results1['State'];
		$zip = $results1['Zip'];
		$total = $_SESSION['order_total'];
		//create order
		$query2 = mysql_query("INSERT INTO Order_Table(OrderDate, OrderCost, OrderPrice, OrderStatus, StreetAddress, City, State, Zip)
							   VALUES('$date', 99.99, $total, 0, '$address', '$city', '$state', $zip)", $connection);
		if($query2){
			$orderID =  mysql_insert_id();
			foreach ($_SESSION['cart_array'] as $cart_itm){
				$productSKU = $cart_itm[0];
				$quantity = $cart_itm[1];
				//add products to order
				$query3 = mysql_query("INSERT INTO Order_Product_Link_Table(OrderID, ProductSKU, Quantity)
									   VALUES('$orderID', '$productSKU', '$quantity')", $connection);
			}
			// link customer and order
			if($query3){
				$query4 = mysql_query("INSERT INTO Customer_Order_Link_Table(CustomerID, OrderID)
									   VALUES('$customerID', '$orderID')", $connection);
				if ($query4) {
					$query5 = mysql_query("INSERT INTO Payment_Table(CustomerID, OrderID, CardType, CardNumber, ExperationDate, SecurityCode, Zip)
									       VALUES($customerID, $orderID, '$cardtype', '$cardnumber', $experation, $securitycode, $zip)", $connection);
					if($query5){
						unset($_SESSION['cart_array']);
						unset($_SESSION['order_total']);
						$message = "Your order has been placed. Your Order Number is: " .$orderID;
					} else {
						$message = "Error: Payment";
					}
				} else {
					$message = "Error: Customer_Order_Link";
				}
			} else {
				$message = "Error: Order_PRoduct_Link";
			}
		} else {
			$message = "Error: Order";
		}
...
<form action="" method="post">
  ...
  <div class="container">
    ...
    <input type="radio" name="cardtype" value="Visa" checkedrequired>Visa<br>
	<input type="radio" name="cardtype" value="Master Card"required>Master Card<br>
	<input type="radio" name="cardtype" value="American Express"required>American Express<br>
  
    <label><b>Name on Card</b></label>
	<br>
	<input type="text" placeholder="" name="nameoncard" required>
	<br>
	
    <label><b>Card Number</b></label>
	<br>
    <input type="text" placeholder="" name="cardnumber" required>
    <br>
	
	<input type="hidden" name="month" value="Month"required>
	<label for="month"><b>Experation Month</b></label>
	...
	<label for="year"><b>Experation Year</b></label>
	...
	<br>
    <input type="text" placeholder="" name="securitycode" required>
    <br>
